...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponseDto approveBooking(long userId, long bookingId, boolean approved) {
        userRepository.findById(userId).orElseThrow(() -> new ValidationException("Пользователь с id " + userId + " не является владельцем предмета"));
    ...
    public List<BookingResponseDto> getUserBookings(long userId, String state) {
        ...
        List<Booking> bookings = switch (actualState) {
            ...
            case CURRENT ->
                    bookingRepository.findAllByBookerIdAndStartBeforeAndEndAfterOrderByStartDesc(userId, dateTime, dateTime);
            case PAST -> bookingRepository.findAllByBookerIdAndEndBeforeOrderByStartDesc(userId, dateTime);
            case FUTURE -> bookingRepository.findAllByBookerIdAndStartAfterOrderByStartDesc(userId, dateTime);
    ...
    public List<BookingResponseDto> getOwnerBookings(long userId, String state) {
        ...
        List<Booking> bookings = switch (actualState) {
            ...
            case CURRENT ->
                    bookingRepository.findAllByItemOwnerIdAndStartBeforeAndEndAfterOrderByStartDesc(userId, dateTime, dateTime);
            case PAST -> bookingRepository.findAllByItemOwnerIdAndEndBeforeOrderByStartDesc(userId, dateTime);
            case FUTURE -> bookingRepository.findAllByItemOwnerIdAndStartAfterOrderByStartDesc(userId, dateTime);
